...
int main(int argc, char **argv) {
    ...
    if(argc < 6) {
        std::cout << "Usage: \n ./migraphx_node_test <path-to-yoloV4 ONNX model> --image/--tensor <path to image directory/tensor file> --batch-size n" << std::endl;
        return -1;
    }
    ...
    if(!(strcmp(argv[2], "--image"))) {
        ...
        while((pDirent = readdir(pDir)) && bs < batch_size) {
            ...
            cv::Mat input_image, input_image_416x416;
            ...
            cv::resize(square, input_image_416x416, cv::Size(416, 416));
            } 
            ...
            cv::resize(square, input_image_416x416, cv::Size(416, 416));
                ...
                cv::resize(input_image, input_image_416x416, cv::Size(416, 416));
            }

            //preprocess
            cv::Mat RGB_input_image;
            cv::cvtColor(input_image_416x416, RGB_input_image, cv::COLOR_BGR2RGB);  // cv::imread reads the image in order BGR. SO need to convert
